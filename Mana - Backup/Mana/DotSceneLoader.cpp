...
DotSceneLoader::DotSceneLoader() : mSceneMgr( 0 ), mTerrainGroup( 0 ), mOwnTerrainGroup( true ) 
{
...
{
	// This is a hack to avoid the deletion of the terrain group when
	// other code assumes ownership of it.  The other code should delete
	// this stuff some time or wait for program termination to clean up.
	if ( mOwnTerrainGroup ) {
		if ( mTerrainGroup )
		{
...
{
	//$ remove unnecessary processes (lighting, camera, etc.) 
...
{
	Ogre::LogManager::getSingletonPtr()->logMessage( "+++ beginning terrain parse" );
    ...
    lightdir.normalise();
    Ogre::Light* l = mSceneMgr->createLight( "tstLight" ); //$ delete this once level is loaded?
	...
	//$create resource groups for more effecient loading & levels
	...
	//commenting this out will prevent the terrain from being implemented
...
void DotSceneLoader::processTerrainPage( rapidxml::xml_node<>* XMLNode )
{
	...
	Ogre::LogManager::getSingletonPtr()->logMessage( Ogre::String( "+++ loading terrain page " ) + name );
    ...
    if ( Ogre::ResourceGroupManager::getSingleton().resourceExists( mTerrainGroup->getResourceGroup(), name ) )
    {
		...
		Ogre::LogManager::getSingletonPtr()->logMessage( Ogre::String( "+++ terrain page loaded: " ) + name );
    }
}
...
void DotSceneLoader::processCamera( rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent )
{
    ...
    Ogre::String name = getAttrib( XMLNode, "name" );
    Ogre::String id = getAttrib( XMLNode, "id" );
	//$ test this to see if this is what is causing my aspect ration issues
    ...
    if ( !pParent )
    {
        Ogre::SceneNode* pNode = mAttachNode->createChildSceneNode( name ); //intVeiwCamera1
